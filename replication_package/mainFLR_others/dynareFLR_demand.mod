...
// The following are auxililiary variables
    SDF     
    RFR     ${RFR}$             (long_name='Risk-free return')
    Dividend ${Dividend}$       (long_name='Dividend')
    Elambda1                    (long_name='Expected marginal utility')
    CovJElambda1                (long_name='Covariance between MU and J')
    JCheck                      
    JNoCov
    JNoElambda1
    REquity 
...
set_param_value('sigma_RBar',0.00001);
set_param_value('rho_sigma_R',vPar.rho_sigma_R);
set_param_value('rho_e_R',0.913);
...
@#if OptionMoments > 0
//stoch_simul(order=3,periods = 100000, drop = 50000,pruning,k_order_solver,irf=0);
...
@#if OptionIRFAndreasen > 0
save("../Output/IRFs/IRFs_FLR_demand", 'vEMAS', 'vNames', 'mIRFProp_zUncertainty_EMAS', 'mIRFProp_RUncertainty_EMAS', 'vNames_Andreasen', 'mIRFProp_Andreasen_R_EMAS', 'mIRFProp_Andreasen_RVol_EMAS', 'mIRFProp_Andreasen_zVol_EMAS')
@#else
save("../Output/IRFs/IRFs_FLR_demand", 'vEMAS', 'vNames', 'mIRFProp_zUncertainty_EMAS', 'mIRFProp_RUncertainty_EMAS') 
@#endif 
